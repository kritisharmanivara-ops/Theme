...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const stickyText = stickyBtn?.querySelector(".btn-text");
  const originalBtn = document.querySelector('.product-form__submit');

  if (!sticky || !stickyBtn || !originalBtn || !stickyText) return;
  ...
  function syncQuantityToMain() {
    const stickyQty = document.querySelector('#sticky-atc input[name="quantity"]');
    const mainQty = document.querySelector('form[action="/cart/add"] input[name="quantity"]:not(#sticky-atc input)');

    if (!stickyQty || !mainQty) return;

    const value = parseInt(stickyQty.value) || 1;

    mainQty.value = value;
    mainQty.setAttribute('value', value);
...
stickyBtn.addEventListener("click", function () {
  ...
  if (originalBtn.disabled || originalBtn.classList.contains("loading")) return;

  const stickyQty = document.querySelector('#sticky-atc input[name="quantity"]');

  // 🔥 button se hi form pakdo (100% correct)
  const form = originalBtn.closest('form');

  if (!form || !stickyQty) {
    console.log("Form ya sticky qty missing");
    return;
  }


  let mainQty = form.querySelector('input[name="quantity"]');


  if (!mainQty) {
    mainQty = document.createElement("input");
    mainQty.type = "hidden";
    mainQty.name = "quantity";
  ...
  console.log("FINAL QTY SET:", value);
  ...
  function syncUI() {
    ...
    const originalText = originalBtn.querySelector("span")?.textContent?.trim();
    if (originalText) {
      stickyText.textContent = originalText;
    }

    stickyBtn.disabled = originalBtn.disabled;

    const stickySpinner = stickyBtn.querySelector('.loading__spinner');

    if (originalBtn.classList.contains("loading")) {
  ...
  const mutationObserver = new MutationObserver(syncUI);

  mutationObserver.observe(originalBtn, {
    attributes: true,
    childList: true,
    subtree: true
  });
